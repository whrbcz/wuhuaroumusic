...
class UserAuth:
    ...
    def send_verification_email(self, email):
        ...
        try:
            ...
            subject = "五花肉云音乐 - 邮箱验证"
            content = "您的验证码是: {}，有效期5分钟。".format(code)
    ...
    def add_favorite(self, song):
        ...
        # 确保用户数据中有收藏列表
        if "favorites" not in self.users[self.current_user]:
            self.users[self.current_user]["favorites"] = []
        
        # 检查是否已经收藏
        for fav_song in self.users[self.current_user]["favorites"]:
            if fav_song["id"] == song["id"]:
                return False, "歌曲已在收藏列表中"
        
        # 添加到收藏列表
        self.users[self.current_user]["favorites"].append(song)
    ...
    def remove_favorite(self, song_id):
        ...
        # 查找并移除歌曲
        favorites = self.users[self.current_user]["favorites"]
    ...
    def get_favorites(self):
        ...
        return self.users[self.current_user]["favorites"]
    ...
    def is_favorite(self, song_id):
        ...
        # 查找歌曲
        for song in self.users[self.current_user]["favorites"]:
            if song["id"] == song_id:
                return True
...
class NeteaseMusicPlayer:
    ...
    def create_widgets(self):
        ...
        # 搜索结果标签页
        self.search_tab = ttk.Frame(self.left_notebook)
        self.left_notebook.add(self.search_tab, text="搜索结果")
        
        # 热搜榜标签页
        self.hot_search_tab = ttk.Frame(self.left_notebook)
        self.left_notebook.add(self.hot_search_tab, text="热搜榜")
        ...
        # 搜索结果树状视图
        self.song_tree = ttk.Treeview(self.search_tab, columns=columns, show="headings")
        ...
        # 搜索结果滚动条
        scrollbar = ttk.Scrollbar(self.search_tab, orient=tk.VERTICAL, command=self.song_tree.yview)
        ...
        # 热搜榜列表
        hot_search_columns = [("排名", 50), ("歌曲名", 500)]
        self.hot_search_tree = ttk.Treeview(self.hot_search_tab, columns=[col[0] for col in hot_search_columns], show="headings")
        for col, width in hot_search_columns:
            self.hot_search_tree.heading(col, text=col)
            self.hot_search_tree.column(col, width=width, anchor=tk.CENTER if col == "排名" else tk.W)
        
        # 热搜榜滚动条
        hot_scrollbar = ttk.Scrollbar(self.hot_search_tab, orient=tk.VERTICAL, command=self.hot_search_tree.yview)
        hot_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.hot_search_tree.configure(yscrollcommand=hot_scrollbar.set)
        
        self.hot_search_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        # 添加刷新按钮
        refresh_frame = ttk.Frame(self.hot_search_tab)
        refresh_frame.pack(fill=tk.X, pady=5)
        refresh_btn = ttk.Button(refresh_frame, text="刷新热搜榜", command=self.load_hot_search)
        refresh_btn.pack(side=tk.RIGHT, padx=5)
        
        # 绑定双击事件
        self.hot_search_tree.bind("<Double-1>", self.search_hot_song)
        ...
        # 添加收藏按钮
        self.favorite_btn = ttk.Button(control_frame, text="收藏", command=self.toggle_favorite)
        self.favorite_btn.pack(side=tk.LEFT, padx=10)
        self.favorite_btn.config(state=tk.DISABLED)  # 初始禁用
        
        # 添加查看收藏按钮
        self.view_favorites_btn = ttk.Button(control_frame, text="我的收藏", command=self.view_favorites)
        self.view_favorites_btn.pack(side=tk.LEFT, padx=10)
        ...
        # 初始化加载热搜榜
        self.load_hot_search()
    
    def update_user_status(self):
        """更新用户状态显示"""
        if self.auth_manager.logged_in:
            # 移除VIP用户类型显示
            self.user_status_var.set("当前用户: {}".format(self.auth_manager.current_user))
    ...
    def load_hot_search(self):
        """加载热搜榜数据"""
        # 清空现有列表
        for item in self.hot_search_tree.get_children():
            self.hot_search_tree.delete(item)
        
        self.status_var.set("正在加载热搜榜...")
        
        def fetch_hot_search():
            try:
                import re
                # 发送请求获取网页内容
                url = "https://ylsb.netlify.app/"
                headers = {
                ...
                for match in matches:
                    if len(match) >= 2 and match[0].isdigit():
                        ...
                        hot_songs.append((rank, song_name))
                ...
                # 限制最多显示30个热搜项
                hot_songs = hot_songs[:30]
                
                # 在主线程中更新UI
                def update_ui():
                    for rank, song_name in hot_songs:
                        self.hot_search_tree.insert("", tk.END, values=(rank, song_name))
                    
                    if hot_songs:
                        self.status_var.set(f"热搜榜加载完成，共{len(hot_songs)}项")
                    else:
                        self.status_var.set("热搜榜加载失败，未找到歌曲数据")
                
                self.root.after(0, update_ui)
                
            except Exception as e:
                print(f"加载热搜榜失败: {str(e)}")
                # 在主线程中更新错误状态
                self.root.after(0, lambda: self.status_var.set(f"热搜榜加载失败: {str(e)}"))
        
        # 在新线程中执行爬取操作
        threading.Thread(target=fetch_hot_search, daemon=True).start()
    
    def search_hot_song(self, event):
        """双击热搜榜歌曲自动搜索"""
        # 获取选中的项
        item = self.hot_search_tree.selection()
        if not item:
            return
        
        # 获取歌曲名
        values = self.hot_search_tree.item(item[0], 'values')
        if len(values) > 1:
            song_name = values[1]
            
            # 设置搜索框内容
            self.search_entry.delete(0, tk.END)
            self.search_entry.insert(0, song_name)
            
            # 切换到搜索结果标签页
            self.left_notebook.select(self.search_tab)
    ...
    def search_music(self):
        ...
        try:
            ...
            if data.get("code") == 200 and data.get("result"):
                ...
                self.status_var.set("找到 {} 首歌曲".format(len(self.playlist)))
            ...
            messagebox.showerror("错误", "搜索时发生错误: {}".format(str(e)))
    ...
    def update_lyric_display(self, current_time):
        ...
        if new_index != self.current_lyric_index and new_index >= 0:
            ...
            self.lyric_text.tag_add("highlight", "{}.0".format(line_num), "{}.end".format(line_num))
            self.lyric_text.tag_config("highlight", background="#ffffcc", font=("SimHei", 12, "bold"))
            
            # 滚动到当前歌词
            self.lyric_text.see("{}.0".format(line_num))
    
    def get_comments(self, song_id, offset=0, limit=20):
        """获取歌曲评论，支持分页"""
        try:
            url = "{}{}".format(self.comment_api, song_id)
            params = {
                "limit": limit,
                "offset": offset,
                "csrf_token": ""
    ...
    def display_comments(self, append=False):
        ...
        def insert_comment_with_animation(index):
            if index < len(self.comments):
                ...
                # 使用format方法替代f-string
                self.comment_text.insert(tk.END, "{}. {} ({})\n".format(index+1, user, time_str))
                self.comment_text.insert(tk.END, "   {}\n".format(content))
                self.comment_text.insert(tk.END, "   点赞: {}\n\n".format(likes))
                
                # 小延迟创建动画效果
                self.root.after(30, insert_comment_with_animation, index + 1)
            else:
                # 所有评论插入完成后
                # 使用format方法替代f-string
                self.comment_count_var.set("评论数量: {}/{}".format(len(self.comments), self.total_comment_count))
                self.comment_text.config(state=tk.DISABLED)
        ...
        # 开始评论动画插入
        insert_comment_with_animation(start_index)
    ...
    def load_more_comments(self):
        ...
        self.status_var.set("正在加载更多评论...({}/{})".format(len(self.comments), self.total_comment_count))
    ...
    def _load_comments_thread(self, append):
        ...
        self.root.after(0, lambda: self.status_var.set(
            "已加载 {}/{}".format(len(self.comments), self.total_comment_count)
        ))
    ...
    def download_song_to_temp(self, song_url):
        ...
        try:
            ...
            print("下载歌曲到临时文件失败: {}".format(str(e)))
    ...
    def play_song(self, index):
        ...
        if 0 <= index < len(self.playlist):
            try:
                ...
                song_name = song["name"]
                artists = "/".join([artist["name"] for artist in song["artists"]])
                self.now_playing_var.set("正在播放: {} - {}".format(song_name, artists))
                self.status_var.set("正在加载: {}".format(song_name))
                ...
                if self.song_url:
                    ...
                    self.status_var.set("正在播放: {}".format(song_name))
                ...
                messagebox.showerror("错误", "播放失败: {}".format(str(e)))
    ...
    def play_with_system_player(self):
        ...
        try:
            ...
            messagebox.showerror("错误", "系统播放器启动失败: {}".format(str(e)))
            # 自动切换回pygame模式并尝试播放
    ...
    def toggle_play(self):
        ...
        if self.playing:
            if self.paused:
                ...
                self.paused = False
                self.play_btn.config(text="暂停")
                self.status_var.set("正在播放: {}".format(self.current_song['name']))
            else:
                # 暂停播放
                if not self.use_alternative_player:
                ...
                self.paused = True
                self.play_btn.config(text="播放")
                self.status_var.set("已暂停: {}".format(self.current_song['name']))
        else:
    ...
    def download_selected(self):
        ...
        try:
            ...
            song_name = song["name"]
            artists = "/".join([artist["name"] for artist in song["artists"]])
            default_filename = "{} - {}.mp3".format(song_name, artists)
            ...
            # 下载歌曲
            self.status_var.set("正在下载: {}".format(song_name))
            ...
            with open(save_path, 'wb') as f:
                for chunk in response.iter_content(chunk_size=block_size):
                    if chunk:
                        ...
                        self.status_var.set("正在下载: {} ({:.1f}%)".format(song_name, progress))
            
            self.status_var.set("下载完成: {}".format(song_name))
            messagebox.showinfo("成功", "歌曲已下载至:\n{}".format(save_path))
            
        except Exception as e:
            messagebox.showerror("错误", "下载失败: {}".format(str(e)))
            self.status_var.set("下载失败")

    def toggle_favorite(self):
        """切换歌曲的收藏状态"""
        if not self.current_song:
            messagebox.showwarning("提示", "请先选择或播放一首歌曲")
            return
        
        # 检查是否登录
        if not self.auth_manager.logged_in:
        ...
        # 检查是否已收藏
        if self.auth_manager.is_favorite(song_id):
            # 取消收藏
            success, msg = self.auth_manager.remove_favorite(song_id)
            if success:
            ...
            # 添加收藏
            success, msg = self.auth_manager.add_favorite(self.current_song)
            if success:
    ...
    def view_favorites(self):
        ...
        # 获取收藏列表
        favorites = self.auth_manager.get_favorites()
        
        if not favorites:
            messagebox.showinfo("提示", "您的收藏列表为空")
            return
        ...
        favorite_tree = ttk.Treeview(dialog, columns=columns, show="headings")
        for i, col in enumerate(columns):
            favorite_tree.heading(col, text=col)
            favorite_tree.column(col, width=columns_widths[i], anchor=tk.CENTER if i in [0, 4] else tk.W)
        ...
        for i, song in enumerate(favorites):
            ...
            artists = "/".join([artist["name"] for artist in song["artists"]])
            album = song["album"]["name"]
            duration = self.format_time(song["duration"] // 1000)
            favorite_tree.insert("", tk.END, values=(i+1, song_name, artists, album, duration))
        
        # 添加滚动条
        scrollbar = ttk.Scrollbar(dialog, orient=tk.VERTICAL, command=favorite_tree.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        favorite_tree.configure(yscrollcommand=scrollbar.set)
        favorite_tree.pack(fill=tk.BOTH, expand=True)
        
        # 双击播放歌曲
        def play_favorite(event):
            selected_item = favorite_tree.selection()
            if selected_item:
                index = int(favorite_tree.item(selected_item[0], "values")[0]) - 1
                # 设置播放列表为收藏列表
                self.playlist = favorites
                self.play_song(index)
                dialog.destroy()
        
        favorite_tree.bind("<Double-1>", play_favorite)
    ...
    def update_favorite_button(self):
        ...
        if self.auth_manager.logged_in:
            ...
            # 检查是否已收藏
            if self.auth_manager.is_favorite(self.current_song["id"]):
                self.favorite_btn.config(text="已收藏")
            else:
                self.favorite_btn.config(text="收藏")
        else:
        ...
        if hasattr(widget, 'attributes'):
            ...
            def animate(step):
                if step <= steps:
                    ...
                    self.root.after(int(step_duration), animate, step + 1)
                else:
    ...
    def fade_out(self, widget, duration=500, start_alpha=1.0, end_alpha=0.0):
        """淡出动画"""
        if hasattr(widget, 'attributes'):
            # 窗口部件
            widget.attributes('-alpha', start_alpha)
    ...
    def slide_in(self, widget, direction='right', duration=500):
        """滑入动画"""
        widget.pack_forget()
        widget.pack()
        widget.update()
        ...
        # 初始位置
        if direction == 'right':
            widget.place(x=x + width, y=y)
        elif direction == 'left':
            widget.place(x=x - width, y=y)
        elif direction == 'down':
            widget.place(x=x, y=y - height)
        elif direction == 'up':
            widget.place(x=x, y=y + height)
        ...
        if direction in ['right', 'left']:
            # 水平滑动
            distance = width
            start_pos = x + width if direction == 'right' else x - width
            end_pos = x
            
            def animate(step):
                if step <= steps:
                    current_x = start_pos + (end_pos - start_pos) * step / steps
            ...
            # 垂直滑动
            distance = height
            start_pos = y - height if direction == 'down' else y + height
            end_pos = y
            
            def animate(step):
                if step <= steps:
                    current_y = start_pos + (end_pos - start_pos) * step / steps
    ...
    def apply_widget_animation(self, widget, animation_type='fade', **kwargs):
        """应用动画到部件"""
        if animation_type == 'fade':
            self.fade_in(widget, **kwargs)
        elif animation_type == 'slide':
            self.slide_in(widget, **kwargs)
    
    def window_fade_effect(self, start_alpha=0.0, end_alpha=1.0, duration=1000):
        """窗口淡入淡出效果"""
        ...
        def animate(step):
            if step <= steps:
                self.current_alpha = start_alpha + alpha_step * step
...
if __name__ == "__main__":
    ...
    # 应用窗口淡入效果
    app.window_fade_effect(start_alpha=0.0, end_alpha=1.0, duration=1000)
    root.mainloop()